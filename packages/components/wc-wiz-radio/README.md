...
# ✨ Wiz Radio
Radio button é nosso componente de seleção, geralmente utilizado para coletar respostas únicas dos usuários. O componente é usado para seleção de uma única resposta selecionável. Um Radiobutton tem dois estados principais; selecionado e desmarcado. É importante sempre usar um texto de apoio para descrever a finalidade do Radiobutton.
...
|-------------------|-------------|----------|------|---|
| disabled    		| Não	| string	| false	| Campo| Por padrão ele vem disabled
| (returnEvent)="teste($event)"		| Não	| function	| null	| Eventemitter 
|value	| Sim	| string	| null	| Campo padrão value do input <input value="{{value}}"
|name	| Sim	| string	| null	| Campo padrão value do input <input name="{{name}}"
